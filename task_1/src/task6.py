...
if __name__ == "__main__":
	...
	for line in sys.stdin:
		if first_line:
			group = re.match("Employ:\s(.*)\n", line)
			group = re.split(",\s", group.group(1))
			first_line = False
	...
	for node in graph.graph:
		for rest in graph.graph.keys():
			if node == rest:
				continue

			if graph.existEgde(node, rest) and not graph.existEgde(rest, node):
				# rest not answering to the node message
				print(f"{rest} -> {node}")
