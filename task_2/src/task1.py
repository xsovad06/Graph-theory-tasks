...
class Graph:
	...
	def getComponentsAndCheapestWay(self):
		"""From every node create new component(graph) and find the cheapest way."""

		components = []
		cheapest_ways = {}

		for name in self.graph.keys():
			node, value = self.getCheapestEdge(name)
			cheapest_ways[name] = (node, value)
			comp = Graph()
			comp.addNode(name)
			components.append(comp)

		return components, cheapest_ways

	def getMinimumSpanningTree(self):
		"""Continuously connecting components with the cheapest ways to find the minimum spanning spanning tree."""

		components, cheapest_ways = self.getComponentsAndCheapestWay()

		for source, way in cheapest_ways.items():
			dest = way[0]
			value = way[1]
...
if __name__ == "__main__":
	...
	min_spanning_tree = graph.getMinimumSpanningTree()
	min_spanning_tree.printGraphUniquePairs()
